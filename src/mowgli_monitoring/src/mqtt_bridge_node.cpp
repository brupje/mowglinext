...
StubMqttClient::StubMqttClient(rclcpp::Logger logger)
: logger_(logger)
{}
...
bool StubMqttClient::subscribe(
  const std::string & topic,
  MessageCallback /*callback*/) noexcept
{
...
struct MosquittoMqttClient::Impl
{
  ...
  static void on_connect_cb(mosquitto * /*mosq*/, void * userdata, int rc)
  {
    ...
    if (rc == 0) {
      self->connected = true;
      RCLCPP_INFO(self->logger, "MQTT connected to %s:%d",
        self->config.host.c_str(), self->config.port);
    } else {
...
: impl_(std::make_unique<Impl>())
{
  ...
  impl_->mosq = mosquitto_new(
    impl_->config.client_id.c_str(),
    true,   // clean session
    impl_.get());
  ...
  if (!impl_->config.username.empty()) {
    mosquitto_username_pw_set(
      ...
      impl_->config.password.empty() ? nullptr : impl_->config.password.c_str());
  }
...
bool MosquittoMqttClient::connect() noexcept
{
  ...
  if (rc != MOSQ_ERR_SUCCESS) {
    RCLCPP_ERROR(
      impl_->logger,
      "mosquitto_connect failed: %s", mosquitto_strerror(rc));
    return false;
  }
  return true;
}
...
bool MosquittoMqttClient::publish(
  const std::string & topic,
  const std::string & payload,
  bool retain) noexcept
{
  ...
  if (rc != MOSQ_ERR_SUCCESS) {
    RCLCPP_WARN(
      impl_->logger,
      "mosquitto_publish failed on '%s': %s", topic.c_str(), mosquitto_strerror(rc));
    return false;
  }
  return true;
}

bool MosquittoMqttClient::subscribe(
  const std::string & topic,
  MessageCallback callback) noexcept
{
  if (!impl_->mosq) {
  ...
  const int rc = mosquitto_subscribe(
    impl_->mosq,
    nullptr,  // message id
    topic.c_str(),
    1 /* QoS */);
  ...
  if (rc != MOSQ_ERR_SUCCESS) {
    RCLCPP_WARN(
      impl_->logger,
      "mosquitto_subscribe failed on '%s': %s", topic.c_str(), mosquitto_strerror(rc));
    return false;
  }
  return true;
}
...
void MosquittoMqttClient::spin_once() noexcept
{
  ...
  if (rc != MOSQ_ERR_SUCCESS && rc != MOSQ_ERR_NO_CONN) {
    RCLCPP_WARN_THROTTLE(
      impl_->logger, rclcpp::Clock{}, 10000,
      "mosquitto_loop error: %s — attempting reconnect", mosquitto_strerror(rc));
...
void MqttBridgeNode::create_subscriptions()
{
  ...
  sub_diagnostics_ = create_subscription<diagnostic_msgs::msg::DiagnosticArray>(
    "/diagnostics", 10,
    [this](diagnostic_msgs::msg::DiagnosticArray::ConstSharedPtr msg) { on_diagnostics(msg); });
...
void MqttBridgeNode::on_mqtt_command(
  const std::string & /*topic*/,
  const std::string & payload)
{
  ...
  int command_int = -1;
  if (std::sscanf(payload.c_str(), "%d", &command_int) != 1 ||
    command_int < 0 || command_int > 255)
  {
    RCLCPP_WARN(
      get_logger(),
...
std::string MqttBridgeNode::serialise_diagnostics(
