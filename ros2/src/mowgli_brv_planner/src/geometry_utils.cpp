...
Polygon2D offset_polygon_inward(const Polygon2D& poly, double dist)
{
  ...
  for (int i = 0; i < n; ++i) {
    ...
    result.push_back({ordered[i].x + bx / blen * scale,
                      ordered[i].y + by / blen * scale});
  }
                        ...
                        const Point2D& b1, const Point2D& b2)
{
  ...
  if (std::abs(d1) < 1e-10 && on_segment(b1, a1, b2)) return true;
  if (std::abs(d2) < 1e-10 && on_segment(b1, a2, b2)) return true;
  if (std::abs(d3) < 1e-10 && on_segment(a1, b1, a2)) return true;
  if (std::abs(d4) < 1e-10 && on_segment(a1, b2, a2)) return true;
