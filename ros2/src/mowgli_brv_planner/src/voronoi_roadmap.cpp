                                        ...
                                        double sample_spacing, int knn)
{
  ...
  for (auto it = vd.vertices().begin(); it != vd.vertices().end(); ++it) {
    ...
    if (!point_in_polygon(pt, boundary)) continue;
  ...
  for (auto it = vd.edges().begin(); it != vd.edges().end(); ++it) {
    ...
    if (it->vertex0() == nullptr || it->vertex1() == nullptr) continue;
    ...
    if (it0 == vert_map.end() || it1 == vert_map.end()) continue;

    int n0 = it0->second;
    int n1 = it1->second;

    // Check edge midpoint is inside boundary and not crossing obstacles
    Point2D mid{(nodes_[n0].x + nodes_[n1].x) / 2,
                (nodes_[n0].y + nodes_[n1].y) / 2};
    if (!point_in_polygon(mid, boundary)) continue;
  ...
  if (knn > 0 && nodes_.size() > 1) {
    ...
    for (int i = 0; i < static_cast<int>(nodes_.size()); ++i) {
      ...
      for (int ki = 0; ki < std::min(k, static_cast<int>(dists.size())); ++ki) {
        ...
        Point2D mid{(nodes_[i].x + nodes_[j].x) / 2,
                    (nodes_[i].y + nodes_[j].y) / 2};
        if (!point_in_polygon(mid, boundary)) continue;
...
{
  if (nodes_.empty()) return {start, end};
  ...
  for (int i = 0; i < static_cast<int>(nodes_.size()); ++i) {
    ...
    if (ds < best_sd) { best_sd = ds; start_idx = i; }
    if (de < best_ed) { best_ed = de; end_idx = i; }
  }
